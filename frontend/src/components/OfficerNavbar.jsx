...
const OfficerNavbar = ({ user, onLogout }) => {
    ...
    <>
      <nav className="fixed top-0 left-0 w-full z-50 bg-[#1f1f23]/80 backdrop-blur-md border-b border-gray-700 px-10 py-4 flex justify-between items-center">
        ...
        <div className="flex gap-8 items-center font-medium">
          <button onClick={() => navigate("/officer")} className={`${isActive("/officer")} hover:text-white`}>
            Dashboard
          </button>
          <button onClick={() => navigate("/officer/tasks")} className={`${isActive("/officer/tasks")} hover:text-white`}>
            My Tasks
          </button>
              ...
              <div className="absolute right-0 mt-3 w-64 bg-[#2a2a2f] text-white shadow-xl rounded-xl p-4 border border-gray-700">
                <div className="flex gap-3 items-center pb-3 border-b border-gray-600">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-purple-400 mt-1">OFFICER</p>
                  </div>
                </div>
                <div className="flex flex-col mt-3 gap-2">
                  <button
                    onClick={() => { navigate("/officer/profile"); setOpen(false); }}
                    className={`p-2 rounded text-left hover:bg-gray-700 ${isActive("/officer/profile") ? "bg-gray-700" : ""}`}
                  >
                    👤 Profile
                  </button>
                  <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-[#2a2a2f] p-6 rounded-xl text-white w-80 text-center shadow-xl">
            ...
            <div className="flex justify-center gap-4 mt-5">
              <button onClick={() => setShowConfirm(false)} className="bg-gray-600 px-4 py-2 rounded hover:bg-gray-500">Cancel</button>
              <button onClick={() => { setShowConfirm(false); onLogout(); }} className="bg-red-500 px-4 py-2 rounded hover:bg-red-600">Logout</button>
            </div>
          </div>
        </div>
      )}
